...
for (user in unique(plot_data$user_short)) {
  ...
  fited_vmaf %>% 
    ...
    ggplot(aes(real_vmaf, model_mos, color = user)) + 
    geom_line(aes(alpha = isHighlighted), size=0.3) +
    ...
    labs(title = "Logistic model of scores versus VMAF", x = "VMAF", y = "Modelled score", color = "User ID") + theme_bw() + scale_alpha_discrete(range = c(0.35, 1), guide = "none") + scale_x_reverse()

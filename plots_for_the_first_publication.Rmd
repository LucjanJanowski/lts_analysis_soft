...
plot_data_raw[plot_data_raw$real_vmaf < 100,] %>%
  ...
  ylim(c(1, 5)) + xlim(c(20, 100)) + theme_bw() + labs(title = "MOS for second phase of the study", x = "VMAF", y = "MOS")
...
for (user in unique(plot_data$user_short)) {
  ...
  fited_vmaf %>% 
    mutate(isHighlighted = (user == '3adab' | user == '88df4' | user == 'bd209')) %>% 
    ggplot(aes(real_vmaf, model_mos, color = user)) + 
    geom_line(aes(linetype = isHighlighted), size=0.3) +
    ...
    labs(title = "Logistic model of scores versus VMAF", x = "VMAF", y = "Modelled score", color = "User ID") + theme_bw() + scale_linetype_manual(values = c("dashed", "solid"), guide = "none")

...
ggplot(heatmap_data, aes(x = external_id, y = hour, fill = Frequency)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5)) +  scale_y_continuous(breaks = seq(0, 24, 2))
```
...
ggplot(df_for_plot, aes(x = external_id, y = mean_correlation, color = weeks)) +
  geom_point() +
  scale_color_gradientn(colors = my_color_palette) +
  labs(x = "User ID", y = "Correlation", title = "Correlation and number of fully watched weeks") +
  theme_bw() +

...
objective_fun <- function(params, df) {
  ...
  df$f_param <- f_param(as.matrix(data_by_weeks[,c("mo", "tu", "we", "th", "fr", "sa")]), a, b)
  
  model <- glm(os ~ f_param, data = df, family = "binomial")  
  
  return(AIC(model))  
}

init_params <- c(3.0, 3.0)
lower_bounds <- c(0.001, 0.001)

opt_result <- optimx(init_params, objective_fun, method="L-BFGS-B", 
                     lower=lower_bounds, df = data_by_weeks)

best_params <- opt_result
print(best_params)

plot(seq(0, 1, 0.01), kumaraswa_den(seq(0, 1, 0.01), best_params$p1, best_params$p2))
